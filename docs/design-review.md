...
[`script2builtins-runtime/docs/design-review.md`](https://github.com/yourorg/script2builtins-runtime/blob/main/docs/design-review.md).
...
1. **The catalog is the spec.** `ALL_APIS` in `src/knowledge` is the
   single source of truth for what counts as a fingerprint surface.
   Adding a surface is a one-line PR to a category file. The walker
   discovers new roots automatically through `watchedRoots()`; the
   matcher resolves them on next run; the runtime package generates
   traps for them on its next build.
...
[CHANGELOG](https://github.com/yourorg/script2builtins/blob/main/CHANGELOG.md#unreleased)
