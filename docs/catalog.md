...
match against an `ApiDefinition` entry in
[`src/knowledge/`](https://github.com/yourorg/script2builtins/blob/main/src/knowledge);
everything `script2builtins-runtime` traps is generated from the same
catalog at build time. Adding a fingerprint surface is a one-line PR.
...
Each is one file in `src/knowledge/<category>.ts` and exports
`<category>Apis: ApiDefinition[]`. The category in the entry
itself doesn't have to match the filename, but conventionally does.
...
[`src/knowledge/index.ts`](https://github.com/yourorg/script2builtins/blob/main/src/knowledge/index.ts)
