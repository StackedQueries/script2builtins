...
export function classifyValue(node: any, aliases: AliasMap, source: string): ValueOrigin | null {
  ...
  // "unknown". We use `null` because the ValueOrigin literal value
  // type doesn't carry an `undefined` variant — downstream consumers
  // treat null as "explicitly absent".
