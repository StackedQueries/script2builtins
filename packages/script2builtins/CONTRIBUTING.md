...
Catalog entries live in the sibling `script2builtins-knowledge`
package — one file per category at
`packages/script2builtins-knowledge/src/<category>.ts`. The shape is:
...
2. If the key uses a new root identifier, add it to `watchedRoots()`
   in `packages/script2builtins-knowledge/src/index.ts`.
